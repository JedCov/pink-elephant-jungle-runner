  ...
  useEffect(() => {
    ...
    function promptText() {
      ...
      if (loop === 0) {
        ...
        if (local < 194) return "Hold Spacebar to Belly-Slide under vines.";
        ...
        <section className="absolute inset-0 z-30 flex items-center justify-center px-6"
          ...
          <div className="w-full max-w-xl rounded-[2rem] p-8 text-center"
            ...
            <div className="mt-6 grid grid-cols-2 gap-2 text-left text-xs text-amber-50/70">
              {[["↑ / W", "Build Charge"], ["← / A   → / D", "Sway the Trail"], ["Tap Spacebar", "Leap"], ["Spacebar", "Hold to slide"], ["ArrowDown / S", "Reverse"], ["Z", "Trunk-Smash"], ["E", "Spin Attack"]].map(([key, label]) => (
                <div key={key} className="flex items-center gap-2 rounded-xl px-3 py-2"
                  style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>
                  <span className="w-24 shrink-0 font-black text-amber-200">{key}</span><span>{label}</span>
                </div>
              ))}
            </div>
